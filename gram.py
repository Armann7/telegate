...
class Gram:
    ...
    async def send(self, receiver: str, msg: Text):
        _logger.debug(f"Send a message '{msg.as_markdown()}' to {receiver}")
        if receiver not in self._idm:
            raise ReceiverNotFound(f'Receiver {receiver} not found')
        await self._bot.send_message(self._idm[receiver], **msg.as_kwargs())
